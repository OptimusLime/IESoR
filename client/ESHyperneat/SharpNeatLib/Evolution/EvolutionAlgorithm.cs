...
namespace SharpNeatLib.Evolution
{
	public class EvolutionAlgorithm
	{
        ...
        public GenomeFilter genomeFilter = new GenomeFilter();
        ...
        AssessGenotypeFunction assessFunction;
		...
		/// Default Constructor.
		/// </summary>
		public EvolutionAlgorithm(Population pop, IPopulationEvaluator populationEvaluator) : this(pop, populationEvaluator, new NeatParameters(), null)//, true)
		{}

		/// <summary>
		/// Default Constructor.
		/// </summary>
        public EvolutionAlgorithm(Population pop, IPopulationEvaluator populationEvaluator, NeatParameters neatParameters, AssessGenotypeFunction assess)//, bool evaluate = true)
		{
            ...
            if(neatParameters.noveltySearch)
            {
                ...
                if(neatParameters.noveltyFixed || neatParameters.noveltyFloat)
                {
			        ...
			        noveltyFixed.updatePopulationFitness(pop.GenomeList);
			        UpdateFitnessStats();
        ...
        public void CalculateNovelty()
        {
            ...
            if(neatParameters.noveltyFixed || neatParameters.noveltyFloat)
            {
					...
					for (int i = 0; i< count; i++)
                    {
                        ...
                        //incorporate the percentage of invidiuals you beat locally
						pop.GenomeList[i].Fitness = fit + neatParameters.localMultiplier*(neatParameters.localCompetition ? pop.GenomeList[i].competition/pop.GenomeList[i].locality : 0);
		...
		//----- Stage 1. Create offspring / cull old genomes / add offspring to population.
			...
			if(neatParameters.multiobjective) {
                ...
                //after ranking, let our genome filter know wassup -- PAUL
                genomeFilter.addPopulation(pop.GenomeList);
		...
		private void CreateOffSpring_Asexual()
		{
			...
			foreach(Species species in pop.SpeciesTable.Values)
			{
				...
				{	// Add offspring to a seperate genomeList. We will add the offspring later to prevent corruption of the enumeration loop.
                    ...
                    while (assessFunction != null && !assessFunction(offspring) && testCount++ < maxTests)
                        offspring = parent.CreateOffspring_Asexual(this);
			...
			for(int genomeIdx=0; genomeIdx<genomeBound; genomeIdx++)
			{
				...
				for(int connectionGeneIdx=0; connectionGeneIdx<connectionGeneBound; connectionGeneIdx++)
				{
					...
					{	// This connection is already registered. Give our latest connection 
						...
						connectionGene.InnovationId = (long)existingConnectionIdObject;
					}
				}
