...
namespace SharpNeatLib.NeuralNetwork.Xml
{
	public class XmlNetworkReaderStatic
	{
		...
		{
            long id = long.Parse(XmlUtilities.GetAttributeValue(xmlNeuron, "id"));
		...
		{
            long sourceNeuronId = long.Parse(XmlUtilities.GetAttributeValue(xmlConnection, "src-id"));
            long targetNeuronId = long.Parse(XmlUtilities.GetAttributeValue(xmlConnection, "tgt-id"));
