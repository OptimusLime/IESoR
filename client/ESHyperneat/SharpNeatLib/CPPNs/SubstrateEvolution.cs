...
namespace SharpNeatLib.CPPNs
{
    public class SubstrateEvolution
    {
     ...
     //   public static int NETWORK_ITERATIONS = 3;
        ...
        public static float SAMPLE_WIDTH = 0.4f;
        ...
        public static float INCREASE_RESSOLUTION_THRESHOLD = 10.5f; //0.03 //TODO add if higher than 0.9 than up to 0.1... and so on...
        ...
        class Rect : IComparable<Rect>
        {
            ...
            public ModularNetwork genome; //reference to the CPPN
                ...
                while (true)    //TODO replace. Do not calculate distance for every rectangle
                {
                    ...
                    if ((Math.Abs(rectList[0].x1 - rectList[0].x2) > minDistance))// || (sampleTreshold != -1.0f && dif > sampleTreshold))
                    {
                        foreach (Rect rect in rectList)
                        {
                            ...
                            rectToAdd.Add(new Rect(fixedx, fixedy, from, rect, 0x0, rect.x1, rect.y1, midx, midy, rect.genome));
                            rectToAdd.Add(new Rect(fixedx, fixedy, from, rect, 0x1, rect.x1, rect.y2, midx, midy, rect.genome));
                            rectToAdd.Add(new Rect(fixedx, fixedy, from, rect, 0x2, rect.x2, rect.y1, midx, midy, rect.genome));
                            rectToAdd.Add(new Rect(fixedx, fixedy, from, rect, 0x3, rect.x2, rect.y2, midx, midy, rect.genome));
                            //rect.visited = true;
                        }
                                                ...
                                                ref  ConnectionGeneList connections, ref List<PointF> hiddenNeurons)
        {
            ...
            uint counter = 0;
            float weight;
            float[] connectionCoordinates = new float[4];

            //CONNECTION DIRECTLY FROM INPUT NODES
            List<PointF> tabuList = new List<PointF>();
            foreach (PointF input in inputNeuronPositions)
            {
                Rect startRec = new Rect(input.X, input.Y, true, null, 0, minX, minY, maxX, maxY, genome);
            ...
            {
                Rect startRec = new Rect(outputPos.X, outputPos.Y, false, null, 0, minX, minY, maxX, maxY, genome);
                startRec.createTree(sampleWidth);
                startRec.addPoints(ref _connections, ref sampleThreshold, ref neighborLevel, ref increaseResolutionThreshold, ref minDistance);
